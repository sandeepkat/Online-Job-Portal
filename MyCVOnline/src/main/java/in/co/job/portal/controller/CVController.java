 ...
 * 
 */
@WebServlet(name = "CVController", urlPatterns = { "/ctl/ChangeCV" })
public class CVController extends BaseCtl {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		log.debug("ChangeCV  doPost method start");
		...
		// get model
		UserModel model = new UserModel();
		CVBean bean = (CVBean) populateBean(request);

		UserBean UserBean = (UserBean) session.getAttribute("user");
		
		String mainSkills = request.getParameter("mainSkills");
		...
		long id = UserBean.getId();
				...
				boolean flag = model.updateCV(id, mainSkills,workExperience,educaTion);
		
				if (flag == true) {
				
					//bean = model.findByLogin(cvBean.getLogin());
					
					session.setAttribute("user", UserBean);
					
					ServletUtility.setBean(bean, request);
					ServletUtility.setSuccessMessage("CV Updated Successfully", request);
				}
		

		ServletUtility.forward(JPView.CV_VIEW, request, response);
		log.debug("ChangeCV  doPost method end");
	}
