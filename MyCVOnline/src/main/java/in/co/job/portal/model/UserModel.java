...
public class UserModel {
	...
	public CVBean findByUserId(long userId) throws ApplicationException {
		log.debug("Model findByLogin Started");
		StringBuffer sql = new StringBuffer("SELECT * FROM CVSKILLS WHERE userId=?");
		CVBean bean = null;
		Connection conn = null;
		System.out.println("sql" + sql);
		...
		try {
			...
			while (rs.next()) {
				...
				bean.setMainSkills(rs.getString(2));
				bean.setEducaTion(rs.getString(3));
				bean.setWorkExperience(rs.getString(4));
			}
			rs.close();
		} catch (Exception e) {
			e.printStackTrace();
	...
	public void updateCVDetails(CVBean bean) throws ApplicationException, DuplicateRecordException {
		log.debug("Model update Started");
		Connection conn = null;

		CVBean beanExist = findByUserId(bean.getUserId());
		// Check if updated LoginId already exist
		if (beanExist != null && (beanExist.getUserId() == bean.getUserId())) {

			try {
				conn = JDBCDataSource.getConnection();
				conn.setAutoCommit(false); // Begin transaction
				PreparedStatement pstmt = conn.prepareStatement(
						"UPDATE CVSKILLS SET mainskills=?,education=?,workexperience=?"
						+ "WHERE userId=?");
				pstmt.setString(1, bean.getMainSkills());
				pstmt.setString(2, bean.getEducaTion());
				pstmt.setString(3, bean.getWorkExperience());
			...
			try {
				...
				conn.setAutoCommit(false); // Begin transaction
				PreparedStatement pstmt = conn.prepareStatement(
						"INSERT INTO CVSKILLS VALUES(?,?,?,?)");
				pstmt.setString(2, bean.getMainSkills());
				pstmt.setString(3, bean.getEducaTion());
				pstmt.setString(4, bean.getWorkExperience());
	...
	public boolean updateCV(long id, String mainSkills, String workExperience, String educaTion) {
		// TODO Auto-generated method stub
		CVBean bean= new CVBean();
		bean.setUserId(id);
		bean.setMainSkills(mainSkills);
		bean.setWorkExperience(workExperience);
		bean.setEducaTion(educaTion);
		try {
			updateCVDetails(bean);
